...
if __name__ == '__main__':
    from models.EEG_feature_extra_EEGPT_ori import *
    model = FeatureExtra()
    device = 'cuda:4'
    ...
    for sub_num in sub_list:
        ...
        for i in range(start, end+1):
            ...
            data = compress_channels_with_pca(data)

            data = zscore(data,1)
            # data_noepoch = einops.rearrange(data, 'b (c d) -> c b d', c=times_second,d=int(raw_data.info['sfreq']))

            x = intercep_eeg(data,samples,-0.5, 0.5, 1024)
            x = x.astype(np.float32)
            x = torch.from_numpy(x).float().to(device)
            y = model(x)
            concate_eeg.append(y)
        concate_eeg = torch.concatenate(concate_eeg)
        # torch.save(concate_eeg, save_dir + f'/eeg_raw_sub{sub_num}_eegptfeature_20250406_bloom1.1.pth')
        torch.save(concate_eeg, save_dir + f'/eeg_raw_sub{sub_num}_eegptfeature_20250407_bloom1.1.pth')
