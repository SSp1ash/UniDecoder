...
def global_compare():
    ...
    for story in story_list:
        ...
        for i in range(start, end + 1):
            rec = np.load(
                config.result_path + f'/largebatch/SMN4Lang_multi/story{story}/SMN4Lang_sub{str(i).zfill(2)}_test_20241212_text.npy')
            gt = np.load(config.project_lfs_path + f'/SMN4Lang/dataset/word_compare_gt/story{story}.npy')
    ...
    results_df.to_csv(config.result_path + '/largebatch/excels/SMN4Lang_multi_singlesubmodel.csv', index=False,
                          encoding='utf-8')
...
def windows_compare():
    ...
    for story in story_list:
        ...
        for i in range(start, end + 1):
            ...
            rec = np.load(
                config.result_path + f'/largebatch/SMN4Lang_multi/story{story}/SMN4Lang_sub{str(i).zfill(2)}_test_{key}_text.npy')
            gt = np.load(config.project_lfs_path + f'/SMN4Lang/dataset/word_compare_gt/story{story}.npy')
            ...
            # 使用pd.concat()将新的一行结果添加到原始DataFrame
            results_df = pd.concat([results_df, result_row], ignore_index=True)
    results_df.to_csv(config.result_path + f'/largebatch/excels/SMN4Lang_multi_singlesubmodel_windows_{key}.csv', index=False,
                      encoding='utf-8')
