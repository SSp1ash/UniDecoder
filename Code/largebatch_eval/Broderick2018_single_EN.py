...
def windows_compare():
    ...
    for story in story_list:
        ...
        # 随机值生成
        # Broderick2018 EN受试者
        story_list = glob.glob(config.result_path + f'/largebatch/Broderick2018_EEG/story{story}/*.npy')
        sub_list = sorted(
            [item for item in story_list if re.search(r'sub\d+', item)],  # 筛选出 sub_EN 开头的路径
            key=lambda x: int(re.search(r'sub(\d+)', x).group(1))  # 提取数字用于排序
        )
        ...
        for sub_path in sub_list:
            ...
            sub_no = re.search(r'sub(\d+)', sub_path).group(1)
            gt = np.load(config.project_lfs_path + f'/Broderick2018/dataset/word_compare_gt/story{story}_EN.npy')
            time = np.load(config.project_lfs_path + f'/Broderick2018/dataset/word_times_bloom_en/word_times_story{story}.npy')
    ...
    results_df.to_csv(config.result_path + '/largebatch/excels/Broderick2018/EEG_singlesubmodel_windows_EN.csv', index=False,
                      encoding='utf-8')
