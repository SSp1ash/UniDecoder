...
def cal_wer_cn(reference, hypothesis):
    """
    Calculate the Word Error Rate (WER) between reference and hypothesis.
    Parameters:
        reference (str): Reference sentence.
    ...
    wer_value = d[r_len][h_len] / r_len
    return wer_value
def calculate_bleu_cn(reference, candidate):
    # 使用jieba对中文字符串进行分词
...
def calculate_meteor_cn(reference, candidate):
    # 使用jieba对中文字符串进行分词
    reference_tokens = list(jieba.cut(reference))
    candidate_tokens = list(jieba.cut(candidate))
...
def windows_compare():
    ...
    # lan = 'EN'
    lan = 'FR'
    # path = f'/exp6/LPPC-fMRI/{lan}/noreduce/'
    # path = f'/exp6/LPPC-fMRI/{lan}/reduce/'
    # path = f'/exp6/LPPC-fMRI/{lan}/unfairness1/'
    # path = f'/exp6/LPPC-fMRI/{lan}/unfairness2/'
    path = f'/exp6/LPPC-fMRI/{lan}/unfairness3/'


    # path = '/exp6/LPPC-fMRI/noreduce/'
    # path = '/exp6/LPPC-fMRI/reduce/'
    # path = '/exp6/LPPC-fMRI/unfairness1/'
    # path = '/exp6/LPPC-fMRI/unfairness2/'
    # path = '/exp6/LPPC-fMRI/unfairness3/'
    ...
    for story in story_list:
        ...
        # 随机值生成
        # LPPC-fMRI CN受试者
        story_list = glob.glob(config.result_path + path + f'*{date}*.npy')
        sub_list = story_list
        ...
        for sub_path in sub_list:
            ...
            # sub_no = re.search(r'_sub(\d+)_test', sub_path).group(1)
            base_path = os.path.basename(sub_path)
            sub_no = base_path[13:18]


            gt = np.load(config.project_lfs_path + f'/LPPC-fMRI/dataset/word_compare_gt/story{story}_{sub_no[:2]}.npy')[:-4]
            time = np.load(config.project_lfs_path + f'/LPPC-fMRI/dataset/word_times_bloom_{(sub_no[:2]).lower()}/word_times_storyrun{story}.npy')[:-4]
            ...
            for win_no in range(rec_windows_str.__len__()):
                ...
                if sub_no[:2] == 'CN':
                    bleu = calculate_bleu_cn(reference_text, candidate_text)
                    wer = cal_wer_cn(reference_text, candidate_text)
                    meteor = calculate_meteor_cn(reference_text, candidate_text)
                else:
    ...
    results_df.to_csv(config.result_path + path + file_name + f'_result{date}.csv', index=False,
                      encoding='utf-8')
