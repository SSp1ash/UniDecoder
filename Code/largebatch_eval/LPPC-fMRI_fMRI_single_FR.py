...
def windows_compare():
    ...
    for story in story_list:
        ...
        sub_list = sorted(
            [item for item in story_list if re.search(r'subFR\d+', item)],  # 筛选出 sub_FR 开头的路径
            key=lambda x: int(re.search(r'subFR(\d+)', x).group(1))  # 提取 FR 后面的数字用于排序
        )
        ...
        for sub_path in sub_list:
            ...
            sub_no = re.search(r'sub(FR\d+)', sub_path).group(1)
            gt = np.load(config.project_lfs_path + f'/LPPC-fMRI/dataset/word_compare_gt/story{story}_FR.npy')[:-4]
    ...
    results_df.to_csv(config.result_path + '/largebatch/excels/LPPC-fMRI/fMRI_singlesubmodel_windows_FR.csv', index=False,
                      encoding='utf-8')
