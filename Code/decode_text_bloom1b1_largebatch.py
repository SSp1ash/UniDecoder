...
def contra_eb_char_level(extend_embs, gt, indexs, device):
    ...
    # # 1. 计算余弦相似度
    # cosine_sim = F.cosine_similarity(extend_embs, gt[:,indexs])  # (n,)
    #
    # # 2. 计算欧氏距离相似度
    # euclidean_distance = torch.norm(extend_embs - gt[:,indexs], dim=1, p=2)  # 欧氏距离 (n,)
    # euclidean_sim = 1 / (1 + euclidean_distance)  # 转换为相似度 (n,)
    #
    # # 3. 计算切比雪夫距离相似度
    # chebyshev_distance = torch.max(torch.abs(extend_embs - gt[:,indexs]), dim=1)[0]  # 切比雪夫距离 (n,)
    # chebyshev_sim = 1 / (1 + chebyshev_distance)  # 转换为相似度 (n,)
    #
    # # 4. 加权求和
    # # final_similarity = 0.6 * cosine_sim + 0.3 * euclidean_sim + 0.1 * chebyshev_sim  # (n,)
    # final_similarity = 1 * cosine_sim + 0 * euclidean_sim + 0 * chebyshev_sim  # (n,)
    # like = final_similarity
...
def gen_word_level_llama3_avoiderror(referen_eb, model, tokenizer, device, Lang, feature_layer = config.llama3_feature_layer):
    nuc_max = 0
    length = referen_eb.shape[1]
    llama3 = LLaMA3(model, tokenizer, device)
    # contrxt_words 提取特征的时候上下文长度
    ...
    try:
        ...
        return result  # 即使出错，仍返回已有的 result
...
if __name__ == '__main__':
    ...
    for sub_num in sub_list:
        key = f'SMN4Lang_sub{sub_num}_test_fMRI_20250310.pth'
        gt_eb_all = torch.load(config.project_lfs_path + f'/Result/largebatch/SMN4Lang_fMRI/story{story}/{key}')[None, :].to(device)
        # result = gen_word_level_llama3(gt_eb_all, model, tokenizer, device, 'CN', 20)

        # avoid error
        result = gen_word_level_llama3_avoiderror(gt_eb_all, model, tokenizer, device, 'CN', 20)
        res = gt_eb_all.shape[1] - result.__len__()
        if res > 0:
            while 1:
                result2 = gen_word_level_llama3_avoiderror(gt_eb_all[:, -res:], model, tokenizer, device, 'CN', 20)
                result = np.concatenate([result, result2])
                res = gt_eb_all.shape[1] - result.__len__()
                if res == 0: break

        np.save(config.project_lfs_path + f'/Result/largebatch/SMN4Lang_fMRI/story{story}/SMN4Lang_sub{sub_num}_test_20250310_text.npy',result)
