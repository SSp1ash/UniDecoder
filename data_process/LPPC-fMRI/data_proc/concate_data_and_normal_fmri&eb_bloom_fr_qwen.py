...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        fMRI_list = [config.rawdata_path + f'/LPPC-fMRI/ds003643-download/fMRI_data/sub-{sub_num}/' + i for i in
                     fMRI_list]
        ...
        # lan_list = ['CN','EN','FR']
        device = 'cuda:6'
        # model_id = "/home/guoyi/llm_model/qwen-1b1"
        model_id = "/home/guoyi/llm_model/qwen2.5"
        ...
        for lan in lan_list:
            ...
            data_file = np.load(config.project_lfs_path + '/LPPC-fMRI/dataset/Dic_token_id_time_FR.npz')
            ...
            for run, TR_num in TR.items():
                ...
                utils.base.create_folder_if_not_exists(config.project_lfs_path + f'/LPPC-fMRI/dataset/word_times_qwen_fr')
                np.save(config.project_lfs_path + f'/LPPC-fMRI/dataset/word_times_qwen_fr/word_times_story{run}.npy', word_time)
                ...
                eb_downsample = lanczosinterp2D(eb, word_time, TR_times)

                # cal rate
                rate = lanczosinterp2D(np.ones(word_time.shape[0]), word_time, TR_times)

                if i < 8:
                    emb_train_list.append(eb)
                    ...
                    # fmri_test = fMRI
                eb_downsample_list.append(eb_downsample)
                i += 1
            emb_train = np.concatenate(emb_train_list, axis=0)
            emb = np.concatenate([emb_train, emb_test], axis=0)
            emb = torch.from_numpy(emb)

            emb_downsample = np.concatenate(eb_downsample_list, axis=0)

            torch.save(emb,
                       config.project_lfs_path + f'/LPPC-fMRI/dataset/fMRI/eb_qwen1.1_{lan}_split{split}_{layer}layer.pth')
            torch.save(emb_downsample,
                       config.project_lfs_path + f'/LPPC-fMRI/dataset/fMRI/eb_qwen1.1_{lan}_split{split}_{layer}layer_downsample.pth')
            ...
            # torch.save(fmri_resample, config.project_lfs_path + f'/LPPC-fMRI/dataset/fMRI/fMRI_raw_sub{sub_num}_qwen_roi_align4s.pth')
            torch.save(fmri_resample,
                       config.project_lfs_path + f'/LPPC-fMRI/dataset/fMRI/fMRI_raw_sub{sub_num}_qwen_roi.pth')
