...
from models.eegpt_pretrain import *

class BrainEncoder(nn.Module):
    def __init__(self, output_dim=1536, dropout_rate=0.2, num_subjects=1):
        super(BrainEncoder, self).__init__()
        self.net1 = RidgeRegression(16*512, 1536)
...
if __name__ == '__main__':
    ...
    model = BrainEncoder().to(device)
    total_params = sum(p.numel() for p in model.parameters())
    print(total_params)

    x = torch.randn([30, 16, 512]).to(device)
    t1 = time.time()
    x = model(x).to(device)
    print(time.time() - t1)
    print(x.shape)
