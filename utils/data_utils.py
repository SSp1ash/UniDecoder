...
# def zscore(mat):
#     r_mean, r_std = mat.mean(1), mat.std(1)
#     mat_result = np.dot((mat - r_mean), np.linalg.inv(np.diag(r_std)))
#     return mat_result
...
def replace_elements(array, replacements, positions):
    """
    Replaces elements in an ndarray with dataset ndarrays at specified positions.

    Parameters:
    array (ndarray): The original array.
    replacements (list of ndarrays): The arrays to insert into the original array.
    positions (list of ints): The starting indices where the replacements will be inserted.

    Returns:
    ndarray: The modified array with the replacements.
    """
    # Ensure replacements and positions are lists
    ...
    positions, replacements = zip(*sorted(zip(positions, replacements), reverse=True))

    # Perform the replacements
    for pos, rep in zip(positions, replacements):
        # Remove the element at the specified position
        array = np.delete(array, pos)

        # Insert the new elements at the same position
        array = np.insert(array, pos, rep)
